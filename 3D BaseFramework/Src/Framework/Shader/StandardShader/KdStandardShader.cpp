	...
	const Math::Color& colRate, const Math::Vector3& emissive)
{
	...
	{
		//ノード内からボーン情報を取得
		for (auto& nodeIndex : data->GetBoneNodeIndices())
		{
			if (nodeIndex >= KdStandardShader::maxBoneBufferSize)
			{
				assert(0 && "転送できるボーンの上限数を越えました。");
				return;
			}

			auto& dataNode = dataNodes[nodeIndex];
			auto& workNode = workNodes[nodeIndex];
